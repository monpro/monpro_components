...
const defaultProps: AutoCompleteProps = {
  getSuggestions: (inputVal) => states.filter(states => states.value.includes(inputVal)),
  onSelect: jest.fn(),
  placeholder: 'test auto complete'
}

const getSuggestionsWithPromise = jest.fn((inputVal: string) => Promise.resolve(states.filter(states => states.value.includes(inputVal))))

let wrapper: RenderResult, inputNode: HTMLInputElement
describe('test default AutoComplete component', () => {
  beforeEach(() => {
    wrapper = render(<AutoComplete {...defaultProps}/>)
    inputNode = wrapper.getByPlaceholderText('test auto complete') as HTMLInputElement
  })
  ...
  it('test default behavior', async () => {
    ...
    expect(defaultProps.onSelect).toHaveBeenCalledWith({value: 'Alaska', order: 1})
  ...
  it('test keyboard event', async () => {
    ...
    expect(defaultProps.onSelect).toHaveBeenCalledWith({value: 'Alabama', order: 0})
    expect(wrapper.queryByText('Alabama')).not.toBeInTheDocument()
  ...
  it('test different rendered children', async () => {
    cleanup()
    const wrapper = render(<AutoComplete {...defaultProps} renderOption={stateRender}/>)
    inputNode = wrapper.getByPlaceholderText('test auto complete') as HTMLInputElement
    fireEvent.change(inputNode, {target: { value: 'A'}})
    await waitFor(() => {
  ...
  it('test async get suggestions function', async () => {
    cleanup()
    const wrapper = render(<AutoComplete {...defaultProps} getSuggestions={getSuggestionsWithPromise}/>)
    inputNode = wrapper.getByPlaceholderText('test auto complete') as HTMLInputElement
    fireEvent.change(inputNode, {target: { value: 'A'}})
    await waitFor(() => {
      expect(wrapper.queryByText('Alabama')).toBeInTheDocument()
